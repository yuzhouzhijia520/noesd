<template>
    <!-- class="wf-" -->
    <div class="wf-DIY-goods3 clean">
        <el-row v-for="(item,index) in list" :key="index" :class="index>2?'wf-goods-row  lines':'wf-goods-row'">
            <el-col :span="24" >
                <div class="wf-goods-img" >
                    <span class="wf-text-left" v-if="item.advance_sale===1">预售</span>
                    <img :src="item.master_graph" v-if="item.master_graph" :style="{height:newHeight}"/>
                </div>
                <el-row class="bottom">
