    ...
    <div class="wf-mainHForm">
        ...
        <el-row>
            <el-col :span="16" :offset="4" class="wf-main-form">
                <el-tabs v-model="editableTabsValue2" type="card"  @tab-click="addTab(editableTabsValue2)"  @tab-remove="removeTab" >
                    <el-tab-pane
                        v-for="(item, indexe) in editableTabs2"
                        :key="indexe"
                        :label="item.title"
                        :name="item.name"
                        closable
                    >
                        <el-form ref="form" v-model="item.ebody" :rules="rules"  class="wf-form" label-width="120px" v-if="showTab">
                            <el-row v-for="(items, index) in item.ebody" :key="index">
                                <el-col v-if="items.eltype===1">
                                    ...
                                    <renders 
                                        ...
                                        @HFCopyEl="()=>HFCopyEl(index,indexe)">
                                    </renders>
                                </el-col>
                                <el-col v-else>
                                    <renders 
        ...
        <div class="wf-submit">
            ...
            <!-- <router-link to="/wf/admin/shop/markting/hform/addHForm"> 
                <el-button  type="info"  disabled size="mini">上一步</el-button>
            </router-link>
            <router-link to="/wf/admin/shop/markting/hform/addHForm"> 
                <el-button @click="preSubmit"   size="mini">上一步</el-button>
            </router-link>
            <router-link to="/wf/admin/shop/markting/hform/addHForm"> 
                <el-button type="primary" v-if="$route.query.state==0" @click="nextSubmit" size="mini" :loading="loadingBtnNext">下一步</el-button>
            </router-link>
            <router-link to="/wf/admin/shop/markting/hform/addHForm"> 
                <el-button type="info"  disabled size="mini">下一步</el-button>
            </router-link> -->
    ...
    methods: {
        initEditPage(key){
            ...
            hfEdit(data).then((res)=>{
                    if(res.data.code===200&&res.data.data.page_data.length!=0){
                        //添加页数
                        //page_data 共有多少页
                        that.total=res.data.data.page_data.length;
                        that.editableTabs2[0].title='第'+res.data.data.page_data[0]+'页';
                        that.editableTabs2[0].id=res.data.data.page_data[0];
                        that.editableTabs2[0].name=res.data.data.page_data[0]+'';
                        if(res.data.data.page_data.length>1&&key===0){
                            res.data.data.page_data.forEach((itemp,index)=>{
                                if(index!=0){
                    ...
                    }else if(res.data.data.page_data.length===0){
                        ...
                        if(res.data.code===200){
                            if(that.$route.query.param&&that.$route.query.id){
                                if(that.$route.query.param=='edit'){
                                    that.$router.push({path:'/wf/admin/shop/markting/hform/finishForm',name:'finishForm',query:{param:'edit',id:that.$route.query.id}});
                                }
                            }else{
                                that.$router.push({path:'/wf/admin/shop/markting/hform/finishForm',name:'finishForm',query:{id:that.$route.query.id}});
                            }
                        }else if(res.data.code===400){
                            that.$message.warning('标题或选项内容未填写完整');
                        }else{
        ...
        HFCopyEl(index,indexe){
            // this.editableTabs2[indexe].ebody.push(this.editableTabs2[indexe].ebody[index]);
            let newEbody=JSON.parse(JSON.stringify(this.editableTabs2[indexe].ebody[index]));
            if(newEbody.obj.key){
                delete newEbody.obj.key;
            }
            if(Object.prototype.toString.call(newEbody.obj.arrType)==='[object Array]'){
                if(newEbody.obj.arrType.length>0){
                    newEbody.obj.arrType.forEach((itemt,indext)=>{
                        if(indext===2){
                            if(Object.prototype.toString.call(itemt.arrType)==='[object Array]'){
                                itemt.arrType.forEach((items,indexs)=>{
                                    if(indexs===1){
                                        items.arrType.forEach((itemy,indexy)=>{
                                            if(itemy.key){
                                                delete newEbody.obj.arrType[indext].arrType[indexs].arrType[indexy].key;
                                            }
                                        })
                                    }
                                })
                        ...
                        if(itemt.key){
                            delete newEbody.obj.arrType[index].key;
                        }
                    })
                }
            }
            this.editableTabs2[indexe].ebody.push(newEbody);
        },
        //单选删除
        HFdelete(index,indexe){
            this.editableTabs2[indexe].ebody.splice(index,1);
            // let newEbody=JSON.parse(JSON.stringify(this.editableTabs2[indexe].ebody[index].obj.arrType[2].arrType[1].arrType[i]));
            // this.editableTabs2[indexe].ebody[index].obj.arrType[2].arrType[1].arrType.push(newEbody)
        },
        //单选子项删除
        ...
        addTab(targetName) {
            ...
            if(targetName==='last'){
                let tabName=this.editableTabs2.length+1+'';
                this.editableTabs2.push({
