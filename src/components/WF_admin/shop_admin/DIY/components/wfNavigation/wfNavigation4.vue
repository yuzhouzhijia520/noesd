<template>
    <!-- class="wf-" -->
    <div class="wf-DIY-wfNavigation4">
        <div v-for="(item,i) in newComProps.item" :key="item.order">
        <el-card class="box-card"   v-if="newComProps.item">
                ...
                <el-row>
                    ...
                    <el-col :span='18' class="wf-card-Props">
                        <el-row>
                            ...
                            <el-col :span='18'>
                                <el-input size="mini" v-model="item.goodsName" @keyup.native="handleTitleInput(item)"></el-input>
                            </el-col>
                        </el-row>
                        <el-row>
                            <el-col :span='6'>
                            ...
                            <!-- <el-dropdown split-button type="primary" size="mini" >
                                请选择跳转链接
                                <el-dropdown-menu slot="dropdown">
                                        <el-dropdown-item v-for="(itemLink,index) in linkList" :key="index" @click.native="handleClick(itemLink.nameEn,item.order,index)" > {{itemLink.nameCh}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown> -->
                            <!-- <el-select  v-model="optionsVal['optionsVal'+i]"  placeholder="请选择跳转链接" size="mini" >
                                <el-option
                                v-for="(itemLink,index) in linkList" :key="index" @click.native="handleClick(itemLink.nameEn,item.order,index,item)"
...
export default {
    ...
    methods: {
        handleTitleInput(val){
            let that=this;
            this.uploadImageOrder = val.order;  //轮播图选框标志
            this.newComProps.item.some((el,i) => {
                if(el.order===val.order){
                    that.newComProps.item[i].goodsName=widthCheck(val.goodsName,8);;
        ...
        addCard(){
            this.newComProps.item.push(
                 {
                    srcUrl:'', //轮播图地址
                    order:Math.max.apply(Math,this.newComProps.item.map((el)=>el.order))+1, //排序
                    imgLink:'', //轮播图跳转路径
                    name:'', //跳转链接地址名称
                    goodsName: "",
                    linkListId:0
