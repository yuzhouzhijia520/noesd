  ...
  <div class="transfer" id="transfer" :style="{width,height}">
    ...
    <template v-if="mode == 'transfer'">
      ...
      <div class="transfer-left">
        <h3 class="transfer-title top">
          <el-button type="text" size="mini" @click="handleformOpenAll" >{{formOpenAll?"全部折叠":"全部展开"}}</el-button>
        </h3>
        ...
        <div class="transfer-main">
          ...
          <el-tree ref='from-tree' :data="self_from_data" show-checkbox :node-key="node_key" @check='fromTreeChecked' :default-expanded-keys="from_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeFrom" :default-expand-all="formOpenAll" :render-content='renderContent' :default-checked-keys="defaultCheckedKeys">
          </el-tree>
        </div>
      </div>
      <!-- 穿梭区 按钮框 -->
      ...
      <div class="transfer-right">
        <h3 class="transfer-title top">
          <el-button type="text" size="mini" @click="handletoOpenAll" >{{toOpenAll?"全部折叠":"全部展开"}}</el-button>
        </h3>
        <h3 class="transfer-title">
        ...
        <div class="transfer-main">
          ...
          <el-tree slot='to' ref='to-tree' :data="self_to_data" show-checkbox :node-key="node_key" @check='toTreeChecked' :default-expanded-keys="to_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeTo" :default-expand-all="toOpenAll" :render-content='renderContent'>
          </el-tree>
        </div>
      </div>
    </template>
    <template v-else-if="mode == 'addressList'">
      ...
      <div class="transfer-left">
        ...
        <div class="transfer-main">
          ...
          <el-tree ref='from-tree' :data="self_from_data" show-checkbox :node-key="node_key" @check='fromTreeChecked' :default-expanded-keys="from_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeFrom" :default-expand-all="formOpenAll" :render-content='renderContent'>
          </el-tree>
        </div>
      </div>
      <!-- 穿梭区 按钮框 -->
      ...
      <div class="transfer-right">
        <div class="transfer-right-item" :class="{'transfer-right-small':move_up}">
          ...
          <h3 class="transfer-title">
            ...
            <img class="move_up_img move_down_img" v-else src="@/assets/admin_img/shop_admin_img/DIY/shopImg/shang.png" alt="" @click="moveUp('down')">
          </h3>
          <!-- 内容区 -->
        ...
        <div class="transfer-right-item" :class="{'transfer-right-small':!move_up}">
          <h3 class="transfer-title">
            ...
            <img class="move_up_img" v-else src="@/assets/admin_img/shop_admin_img/DIY/shopImg/shang.png" alt="" @click="moveUp('up')">
          </h3>
          <!-- 内容区 -->
          <div class="transfer-main" v-if='move_up'>
            <!-- <slot name='to'></slot> -->
...
export default {
  ...
  data () {
    return {
      ...
      formOpenAll:false, //是否全部展开
      toOpenAll:true, //是否全部展开
    }
  },
  props: {
    // 宽度
  ...
  methods: {
    ...
    addToAims () {
      ...
      function inquireIsExist (item, strData = self_to_data) {
        ...
        let existed = reg.test(strData)
          // for (let i of data) {
          //   if (item[that.defaultProps.pid] == i[that.defaultProps.pid]) {
          //     existed = true;
      ...
      arrayCheckedNodes.forEach(item => {
        recursive(item, that.self_from_data);
      });
      // 源树递归查询
      function recursive(obj, data) {
        for (let i of data) {
          debugger
          if (i[that.defaultProps.pid] == obj[that.defaultProps.pid]) {
            let arr = data.filter(b => b[that.defaultProps.pid] != obj[that.defaultProps.pid]);
    ...
    handleformOpenAll(){
        this.formOpenAll=(this.formOpenAll?false:true);
    },
    handletoOpenAll(){
        this.toOpenAll=(this.toOpenAll?false:true);
    }
  },
  watch: {
    // 左侧 状态监测
    ...
    filterListFirst (newval, oldval) {
      ...
      this.addressee = this.addressee.filter(item => reg.test(item[this.defaultProps.label]))
    },
    // 通讯录模式 右2筛选
    filterListSecond (newval, oldval) {
      ...
      let reg = RegExp(newval)
      this.Cc = this.Cc.filter(item => reg.test(item[this.defaultProps.label]))
    },
    // 通讯录模式 右3筛选
    ...
    // 监听展开 折叠
    formOpenAll(newval,oldval){
        for(var i=0;i<this.$refs['from-tree'].store._getAllNodes().length;i++){
           this.$refs['from-tree'].store._getAllNodes()[i].expanded=newval;
        }
    },
    // 监听展开 折叠
    toOpenAll(newval,oldval){
        for(var i=0;i<this.$refs['to-tree'].store._getAllNodes().length;i++){
           this.$refs['to-tree'].store._getAllNodes()[i].expanded=newval;
        }
    },
  }
}
...
.transfer-left .top,.transfer-right .top{
  border: none;
